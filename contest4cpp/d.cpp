...
class RMQ {
 public:
  explicit RMQ(const std::vector<int64_t> &vector) {
    ...
    for (size_t i = 1; i < sparse_table_.size(); ++i) {
      ...
      for (size_t j = 0; j < sparse_table_[i].size() - degree_[i - 1]; ++j) {
        sparse_table_[i][j] = std::gcd(sparse_table_[i - 1][j], sparse_table_[i - 1][j + degree_[i - 1]]);
      }
    }
  }
  void Get(const size_t &left, const size_t &right) {
    ...
    std::cout << std::gcd(sparse_table_[max_index][left], sparse_table_[max_index][right - degree_[max_index] + 1])
              << "\n";
  }
